...
#define MOCK(x) ((splay_node*) x)

#define assert_node(expect,found_ptr) do { \
	const splay_node expected = expect; \
	const splay_node got = *(found_ptr); \
...
static void test_right_path_4() {
	...
	splay_node a = { .key = 100, .left = MOCK(1), .right = &b };

	splay_tree this = { .root = &a };
	splay(&this, 400);

	assert(this.root == &d);
	assert_node(((splay_node) { .key = 400, .left = &a, .right = MOCK(5) }), &d);
...
static void test_left_path_5() {
	...
	splay_node b = { .key = 900, .left = &c, .right = MOCK(5) };
	splay_node a = { .key = 1000, .left = &b, .right = MOCK(6) };

	splay_tree this = { .root = &a };
	splay(&this, 600);

	assert(this.root == &e);
	assert_node(((splay_node) { .key = 600, .left = MOCK(1), .right = &b }), &e);
	assert_node(((splay_node) { .key = 900, .left = &d, .right = &a }), &b);
