...
\def\Abstract{%
	...
	data structures implementing ordered and unordered dictionaries.

	The performance of modern computers is not modelled very closely by
	traditional models (e.g.\ Turing machines or RAM). Many data structures
	have been developed to exploit the memory hierarchy. It is well known
	that cache-aware data structures, like B-trees, perform very well
	on hard drives. To gauge the benefits that can be gained by cache-aware
	data structures in main memory, we benchmark B-trees and cache-oblivious
	B-trees.

	Many dictionary data structures are self-adjusting, and such data
	structures are difficult to reconcile with block-level accesses
	...
	We explore the performance benefits of self-adjusting by comparing
	splay trees, $k$-splay trees and $k$-forests with non-self-adjusting
	structures.
}
