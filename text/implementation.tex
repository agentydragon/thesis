...
Each synthetic experiment setup has a tunable \textit{size} $S$
...
We used data from \textit{Extended Edited Synoptic Cloud Reports from Ships and
...
locality-preserving mapping called the \textit{Z-order} (or
\textit{Morton order} after its inventor, who introduced it in
...
the coordinates.
A possible alternative would be using a \textit{Hilbert curve}
...
Because stations at one position usually give multiple reports, the key
for our dictionary also needs to contain a unique identifier of the report,
which we create by counting the reports from each unique position.
