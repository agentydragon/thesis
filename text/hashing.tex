...
k\in\mathcal{U}: \Pr[h(k)=i]=\frac{1}{M}$, so $\E[C_i]=\frac{N}{M}$, which is
constant if we maintain $M=\O(N)$, so the expected time per operation is also
constant.  The $\frac{N}{M}$ ratio is commonly called the \emph{load factor}.

However, storing a random hash function would require $|\mathcal{U}|\log M$
bits, which is too much if we only store a few keys from a very large universe.
...
\item \emph{Simple tabulation hashing} (\cite{univ-classes}):
	...
	model and it needs space $\O(c\cdot |\mathcal{U}|^{1/c})$.  % TODO: approx u^\epsilon
...
\begin{figure}
	...
	% Obtaining free slot
	\node (obtaining-label) at (7.2,3.5) {Free up $L[h_l(10)]$};
	...
	\textsc{Insert}(10); $h_l(10)=1$, $h_r(10)=2$
...
\item For general $k$-independence, we can use
	...
	\cite{new-hash-fns}), where $p$ is a prime greater than $|\mathcal{U}|$
