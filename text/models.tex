...
% TODO: check tall cache assumption
...
\theorem{Consider an algorithm that causes $Q^\star(n; Z,L)$ cache misses on
a problem of size $n$ using a $(Z,L)$ ideal cache. Then, the same algorithm
incurs $Q(n; Z,L)\leq 2Q^\star(n;Z/2,L)$ cache misses on a $(Z,L)$ cache
that uses LRU replacement.}
\proof{
	% TODO: Sleator-Tarjan: Amortized efficiency of list update and paging
	% rules
}
