...
The cache-oblivious B-tree is composed of three components.
...
root-to-leaf path in order takes $\Theta(\log_B N)$ block transfers.
...
blocks.
Finally, we will combine the two previous parts with \textit{indirection} to
obtain the desired $\Theta(\log_B N)$ time to \textsc{Find}, \textsc{Insert}
and \textsc{Delete}.
...
TODO: referencovany cobt clanek je spatne -- to je dost jina implementace
...
which was invented by Peter van Emde Boas. % TODO: reference. +nebyl to tym?
...
TODO: go-to-left-sibling optimization removing multiplication
TODO: graf
...
developed by Willard\cite{willard92} performs $\Theta(\frac{\log^2 N}{B})$
