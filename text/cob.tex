...
and an array $P[j]$ of van Emde Boas order indices of nodes passed in every
...
$P[D[d]]$, it follows that the van~Emde~Boas index of the current node can be
calculated in $\O(1)$ time as:
$$P[d]=P[D[d]] + T[d] + (i \And T[d]) \cdot B[d]$$

Our root-to-leaf traversal starts by setting $i\gets 0, d\gets 0, P[0]=0$.
...
incrementing the depth $d$ and calculating the new $P[d]$.
...
We can slightly optimize this access pattern by calculating $P[d]$ for $L$
from $P[d]$ for $R$ by simply subtracting $T[d]$, because $B[d]$ will
