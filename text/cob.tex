...
As in cache-aware B-trees, \textsc{Find}s and updates on cache-oblivious
B-trees require $\Theta(\log_B N)$\footnote{
	Strictly speaking, the bound is $\Theta(1+\log_B N)$
...
on any root-leaf path costs $\Theta(\log_B N)$ block transfers (assuming
...
takes $\Theta(\log_B N)$ block transfers (the added $\O(1)$ covers
the $B\geq N$ case).
\end{proof}
...
boostrapping structure in $\Theta(\log_B (N/\log N))$ to find the piece
that may contain the key, and scan it in $\Theta(\frac{\log N}{B})$,
so the time to \textsc{Find} is still $\Theta(\log_B N)$.
...
time for splits and merges, which gives us $\Theta(\log_B N+\frac{\log N}{B})=
